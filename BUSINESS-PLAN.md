...
Grounded in the repo (PLAN.md, README.md, AUDIT.md, docs/architecture.md, GO-LIVE.md) and the 2026 competitive landscape. Today: 2026-05-30. Runway: ~1 month. The AWS+Vercel "H0" hackathon (due Jun 29, 5pm PDT) is a launch ramp, not the goal.
...
3. **The moat and the biggest unfinished gap are the same axis: verification depth.** Today, in live mode, an incident on our *exact* ICP's app (Sentry-connected, zero tests, no repro script) **escalates instead of auto-fixing**. The "it fixed it while you slept" promise currently only fully fires in simulation. Closing that — repro-from-stack-trace + smoke-test generation + error-rate diff — *is* both the roadmap and the durable IP.
...
**The reconciliation (this is a decision, not a seam):** *Position and differentiate on ICP-A; monetize across the A→B graduation.* PLAN.md §1 bets everything on A. The evidence forces a split: the differentiation is most defensible on A, but the *billable business* lives on B.
...
| **WTP / ACV** | Worst; $0–600/yr (they chose v0 *to avoid* paying for engineering) | **Good; $2.4K–9.6K/yr** — a $200–500/mo "on-call you don't have" beats a $150K+ SRE hire | Best per-account; $15K–60K+/yr but long cycle |
...
- Vibe-coded apps are flooding production with no maintainer: **40–62% of AI-generated code has security flaws**, **AI PRs carry 2.74x more vulnerabilities**, and CVEs attributed to AI code went **6 → 35/month from Jan → Mar 2026**. ([Founders Build, Devs Fix](https://dev.to/konst_/founders-build-devs-fix-the-reality-of-vibe-coding-tools-in-2026-3o5))
- **The Moltbook breach (Jan 2026):** an AI-agent app whose founder "didn't write one line of code" leaked **1.5M API tokens + 35k emails** via a client-side Supabase key — the canonical "no human can read this codebase" disaster.
...
|---|---|---|---|---|
| **Watch (free PLG)** | $0 | Vibe-coders (A), trials | **Diagnosis only** — detect, investigate, plain-English root cause. No preview deploy, no panel | Verification capped to $0 |
...
**The codebase is past most people's *finished* hackathon projects** (M0–M12, ~45 tests green, panel reviewer, one-tap revert, audit trail, pgvector memory, scorecard, cheap-provider support). **The entire remaining job is crossing sim→live on ONE real case and packaging it. Do not re-build M0–M12.** The single riskiest thing is that the verify-not-review loop has never run live, and three load-bearing legs are written-but-untested and currently fail-closed to escalate.
...
**Distribution bets, ranked by leverage:**
...
*Synthesized from a 6-agent research workflow (live 2026 market research + repo-grounded analysis). Companion to [PLAN.md](PLAN.md) (the build spec) and [GO-LIVE.md](GO-LIVE.md) (the deploy checklist).*
