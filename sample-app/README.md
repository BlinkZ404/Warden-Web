...
non-technical founder vibe-coded with v0." Nightshift watches it, and when a
production error fires, it diagnoses → fixes → verifies → ships (with one-tap
human approval).

It has no dependencies on purpose: the Nightshift verification gate runs this
...
reproduce a production crash, then lets Nightshift fix it. See
