...
→ 📲 founder gets a readable push → one tap to ship · one tap to revert
...
(Sentry, Claude, OpenAI, Vercel), which are pluggable adapters, not the product.
...
- ✅ **Read-only** investigation · **Fixer** (Claude) · **Reviewer** (GPT) cross-check
- ✅ **verification gate**: runs the target app's *real tests* + a *real reproduction*
...
delivery, the LLMs). The **safety-critical verification stays real**: it
 runs `node --test` and a reproduction script against the patched code.
...
2. **Trust by verification, not review.** The founder can't vet code, so the
 ...
 from different model families cross-check, but **agreement is only a filter; disagreement escalates; it never overrides a failed check** (PLAN §5.3–5.4).
3. **The database is the product.** Aurora is a state machine + event
 log + pgvector memory + scorecard. Every decision is recorded, so the system
 gets safer and smarter over time.

See [docs/product/architecture.md](docs/product/architecture.md) for diagrams.
...
lib/
 ...
 agents/ Investigator · Fixer (Claude) · Reviewer (GPT); sim + live
...
uncertain, escalate; never guess. See [docs/product/plan.md](docs/product/plan.md) §5.
