...
flowchart TB
    ...
    ORCH --> REV[Reviewer · Codex<br/>diff + git-history checks]
    ORCH --> GATE[[Deterministic gate<br/>tests + error-gone + no-new-errors]]
...
```mermaid
flowchart LR
    A[Agents propose] --> B[[Deterministic gate<br/>REAL safety net]]
