...
| **Testing** | Real integration coverage, not mocks of the thing that matters | 102 tests across 22 files against a real Postgres, headlined by an executable end-to-end acceptance spec; the verification gate and its fail-closed behavior (zero tests collected is never a pass) are themselves tested. |
