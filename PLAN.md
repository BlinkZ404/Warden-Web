...
1. Drop this file in the repo root as `PLAN.md`.
2. Tell Claude Code: "Read PLAN.md. Implement Milestone 0 only, then stop and let me review."
...
- **Trust by verification, not code review.** The founder can't vet code, so safety comes from deterministic checks + cheap reversibility, not from them reading a diff.
...
- **Vendor-neutral.** Error sources and agents are pluggable adapters. Sentry, Claude, Codex are interchangeable parts, not the product.
...
- Judging hooks: the DB is a deliberate state machine + event log + vector memory (Technical Implementation); plain-English approval UX (Design); solves a real founder pain (Impact); verify-not-review trust model (Originality).
...
5. **Data mutations are human-only in v1.** Read-only DB investigation is allowed and autonomous. Any write/UPDATE/DELETE requires a dry-run (transaction + rollback) shown to the human in plain English + an Aurora snapshot first. Default to NOT doing data fixes.
...
detected
  ...
  → under_review        (Codex reviews diff + git history; consensus computed)
...
Orchestrator (state-machine driver) ──read/write──▶  Aurora PostgreSQL (Serverless v2)
        ...
        ├─▶ Fixer = Claude
        ├─▶ Reviewer = Codex
        ├─▶ Deterministic gate: tests + preview-error-check
...
See [docs/architecture.md](docs/architecture.md) for the as-built diagram.

## 8. Deployment: not GitHub-centric

"GitHub" bundles three jobs — keep them separate: code store + diff (git); approval artifact (a Vercel preview URL + plain-English summary, NOT a PR); deploy + rollback (Vercel CLI/API). Default path: agent commits to a branch → preview → verify → on approval promote → rollback = `vercel rollback`. A PR mode lives behind the same adapter for teams.
...
**v1 — built end-to-end (one vertical):** Sentry error → investigate (read-only) → Claude fix on a branch → Codex review (incl. git history) → preview deploy + verify → push notification → mobile approval → promote to prod → verify → record outcome + update memory/scorecard → one-tap rollback.
...
M0 Scaffold · M1 Data + state machine · M2 Ingest · M3 Orchestrator skeleton · M4 Investigation (read-only) · M5 Fixer (Claude) · M6 Reviewer (Codex) · M7 Deterministic verification · M8 Approval · M9 Deploy + rollback · M10 Memory/learning · M11 Dashboard · M12 Stubs, seed bugs, README, architecture diagram. **All built; see README "What's built".**
...
4. Open it: plain-English summary + "tested on a preview, the error is gone, no new errors."
...
Next.js (App Router) on Vercel; mobile approval PWA with web push. Amazon Aurora PostgreSQL (Serverless v2) + pgvector. Event-driven orchestration (job table; stateless/resumable). Agents pluggable behind an `Agent` interface (Fixer = Claude; Reviewer = Codex). Integrations: Sentry, GitHub (git), Vercel. Secrets never exposed to the model.

## 16. Open decisions / known risks

Verification depth (reproduce the error against the preview; escalate when not reproducible). No tests in vibe-coded apps (generate a smoke test). Correlated agent blind spots (don't let "both agreed" override a failed deterministic check). Autonomy danger (keep scope conservative, human gate, instant rollback). Cost (watch AWS + token usage).
